...
public partial class Walker
{
    ...
    List<Vector2I> CalculateTileVectors()
    {
        ...
        foreach (var tilePoint in validationPoints)
        {
            ...
            var charLinePositionOnTilemap = 
                FloorToVector2I((Position + tilePoint) / World.TileSize);

            var movementPositionOnTilemap = 
                FloorToVector2I((movementPosition + tilePoint)/ World.TileSize);
            ...
            float xStep = lineDirection.Y == 0 ? 
                0 : (float)(lineDirection.X / lineDirection.Y);
            float yStep = lineDirection.X == 0 ? 
                0 : (float)(lineDirection.Y / lineDirection.X);
            ...
            for (int x = lineDirection.X; x != 0; x += increment)
            {
                var nextPosition = new Vector2I(
                    charLinePositionOnTilemap.X + x, 
                    charLinePositionOnTilemap.Y + (int)(yStep * x));
            ...
            {
                var nextPosition = new Vector2I(
                    charLinePositionOnTilemap.X + (int)(xStep * y), 
                    charLinePositionOnTilemap.Y + y);
    ...
    bool ValidateCollisionForList(List<Vector2I> tilesTouched)
    {
        ...
        foreach (var tileVector in tilesTouched)
        {
            ...
            if (this.Debug)
            {
                if (hasCollision)
                    Logger.LogWarning(
                        $"TileVector: {tileVector.X}, {tileVector.Y}, hascollision");
                else
